...
class LobbyTableLess extends StatelessWidget {
  final GameSelectCall onGameTap;
  final Function onReorder;
  final List<ColumnType> selectedColums;
  final ScrollController controller;
  final ColumnType sortSelectedColumn;
  final ColumnType sortSelectedColumnOld;
  final bool ascending;
  final bool ascendingOld;
  final Function(ColumnType) onHeaderTap;
  ...
  LobbyTableLess(
      ...
      this.onReorder,
      this.selectedColums,
      this.sortSelectedColumn,
      this.sortSelectedColumnOld});
...
class _LobbyTableState extends State<LobbyTable> {
  ...
  Widget getHeader(ColumnType type) {
    return Center(
        child: Text(columnHeader[type],
            style:
                TextStyle(fontSize: 16))); // TODO GestureDetector for sorting
  }
  ...
  void loadWidgets() {
    columnWidget = {
      ...
      ColumnType.Mode: (OnlineGame game) =>
          Center(child: Text(game.isRated ? "Rated" : "Unrated")),
      ColumnType.Fullness: (OnlineGame game) =>
          Center(child: Text(game.player.length.toString() + "/3")),
    };
  }
