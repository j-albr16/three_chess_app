...
class ThreeChessBoard extends StatefulWidget {
  ...
  ThreeChessBoard(
      {this.boardState, this.height, this.width, this.isOffline = false});
...
class _ThreeChessBoardState extends State<ThreeChessBoard> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero).then((_) {
      ...
      playingPlayer = gameProvider.player?.playerColor ??
          PlayerColor.white; // TODO alternative red call will be removed
  ...
  _moveWasMade(context) {
    if (!widget.isOffline) {
      GameProvider gameProvider =
          Provider.of<GameProvider>(context, listen: false);
      gameProvider.sendMove(chessMove: widget.boardState.chessMoves.last);
    }
//TODO make a timefull chessMove
  ...
  Widget build(BuildContext context) {
    ...
    playingPlayer = gameProvider.player?.playerColor ?? PlayerColor.red;

    setState(() {
      // TODO MOVE TO POSITION WHERE HIGHLIGHT CHANGES MAYBE
    ...
    return Listener(
      ...
      onPointerDown: (details) {
        String whatsHit =
            widget.tileKeeper.getTilePositionOf(details.localPosition);
        if (whatsHit != null) {
        ...
        _startAMove() {
          ...
          if (widget.boardState.selectedMove ==
                  widget.boardState.chessMoves.length - 1 ||
              widget.boardState.selectedMove == null) {
            highlighted = MapEntry(whatsHit,
                ThinkingBoard.getLegalMove(whatsHit, widget.boardState));
        ...
        PlayerColor pieceColor =
            widget.boardState.pieces[whatsHit]?.playerColor;
        ...
        setState(() {
          if (highlighted == null) {
            ...
            if (highlighted.value.contains(whatsHit) && myTurn) {
              if (!widget.isOffline) {
                // TODO THIS WILL BE DIFFRENT ONCE THERES A BOARDSTATE MANAGER THAT HANDLES OFFLINE MOVES
                setState(() {
